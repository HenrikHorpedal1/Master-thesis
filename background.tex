...
\section{Technical explanation of a typical Leslie speaker}
Although many models have been released upon the years, the Leslie 122 may be the most famous and share the same principle of operation as several other models \citep{TheatreOrgansMysteryLeslie}.
The basic concept of a Leslie speaker is illustrated in \autoref{fig:leslie-inner-workings}.

\begin{figure}[h]
	\centering
	\scalebox{0.7}{\input{fig/background/leslie-inner-workings.tikz}}
	\caption{Basic concept of a Leslie speaker. Redrawn from \citet{Leslie122Manual1971}.}
	\label{fig:leslie-inner-workings}
\end{figure}

The sound signal from an organ or other instrument enters the Leslie and is amplified by a 40-watt amplifier. Then the signal is split with a 12dB per octave 800 Hz, 16 ohm crossover. The treble frequencies are feed to a \nicefrac{3}{4}$''$ Jensen compression driver while the bass frequencies are fed to a 15 inch woofer, both 16 ohm.

The cabinet usually has three compartments, see \autoref{fig:inside-leslie147}. The upper one houses the treble rotor, the middle houses the speakers and crossover while the lower compartment houses the bass rotor and the amplifier.

\begin{figure}
	\begin{center}
		\includegraphics[width=0.6\textwidth]{fig/background/inside-leslie147.jpg}
	\end{center}
	\caption{inside Leslie 147, from %http://www.highontechnology.tech/2017/05/leslie-147-magic-rotating-speaker-system.html
	}\label{fig:inside-leslie147}
\end{figure}

Both speakers them self are stationary, but plays into rotating deflectors. The compression driver fires up into what starts as vertical tube and flares out into a horn. Although there appear to be two horns, only one is functional, the other is plugged and serves to balance the horn while it spins. A diffuser is located at the end of the horn which will be furthered discussed in section x.

The woofer fires downward into a rotating drum. The drum normally rotates the other direction compared with the horn (dette trenger kilde). The drum has straight walls and a scoop that starts vertically and ends horizontally.

Both rotors are driven with belts and pulleys by two AC induction motors, one for fast and one for slow, giving a total of four motors in the Leslie speaker. DC current is in some configurations applied through the fast speed motor in order to brake the speed quicker.

The speeds are often controlled by a switch that is located on the side of the organ, see figure x.The slow setting is often referred to as CHORALE while the fast setting is referred to as TREMOLO. Giving three speed modes with the BREAK setting.

There exist no official publications of the velocities of the drum and rotor
two speed modes, however \cite{Herrera2009} measured
the rotation rates of a Leslie 44W speaker by feeding audible tones into
the cabinet and extracting the rotation rates from the periodic envelope
of the recorded signal. The results are presented in
Table~\ref{tab:leslie_params}. The drum chorale speed of 0.101 Hz (6.1 RPM)
appears inconsistent with other sources, which consistently report values
in the range of 36--40 RPM \cite{HammondWiki}.

\begin{table}[h]
	\centering
	\begin{tabular}{lcc}
		\hline
		\textbf{Parameter}   & \textbf{Horn}        & \textbf{Drum}        \\
		\hline
		Slow speed (chorale) & 0.672 Hz (40.3 RPM)  & 0.101 Hz (6.1 RPM)   \\
		Fast speed (tremolo) & 6.920 Hz (415.2 RPM) & 5.955 Hz (357.3 RPM) \\
		\hline
	\end{tabular}
	\caption{Measured rotation rates of a Leslie 44W \cite{Herrera2009}.}
...
\subsection{The Leslie Effect}

The rotating horn produces amplitude modulation (AM) and a Doppler-induced frequency modulation (FM) \cite{kronland2008}. In the woofer section, FM is assumed to be negligible, leaving AM as the dominant effect \cite{kronland2008}.

Under free-field conditions, both modulations are approximately sinusoidal, and the resulting percept does not correspond to the typical Leslie effect \cite{kronland2008}. The characteristic sound emerges when cabinet wall reflections are included, giving rise to image sources that interfere with the direct sound and break the periodicity of both modulation laws \cite{kronland2008}. This aperiodic behaviour is visible in measurements made by the authors on a model 122A Leslie cabinet placed in an anechoic chamber and driven with an 800~Hz sinusoidal input. Beyond the cabinet, rotation of both sources produces a time-dependent coupling with the listening room, adding a spatial modulation component \cite{kronland2008}.

(skriv om diffusers i hornene)

\subsection{Modern takes}

\section{Exponential Horn Theory}
...
According to one of Donald Leslie's original patents, \cite{Leslie1949RotatableTremulantSoundProducer}, the horn is preferably of exponential type.
An exponential horn is a horn where the cross-sectional area $S$ increases exponentially along the axis(acustic centerline):
\begin{equation}
	S(x) = S_0\,e^{mx}
	\label{eq:horn-area}
\end{equation}
where $S_0$ is the throat area, $x$ is the distance from the throat \textbf{TODO: noe med acoustic centerline}, and $m$ is the flare constant that controls the rate of expansion. A key property of the exponential horn is its cutoff frequency \citep{Kolbrek2008HornTheoryPart1}:
\begin{equation}
	f_c = \frac{mc}{4\pi}
...
The following derivation is based on \citep{egeland2002modeling}.

\begin{figure}[htbp]
	\centering
	\input{tikz/motor_elastic_load.tex}
...
The elastic transmission is modelled as a spring with spring constant K and damper with damping coefficient D resulting in the torque.
...
by introducing the variable $\theta_r$ the derivation becomes more \textbf{TODO: ett eller annet}.
...
The transfer functions with motor torque as input and angle of the load as output then become
...
Friction belt drives follow the laws of rope friction. Different examples of friction belt drives include flat belt, round belt and V-belt transmission \textbf{TODO: find source confirming V-belt is also a friction belt drive}. When correctly designed, these power transmission systems deliver low slip and an almost constant transmission ratio. \textbf{TODO: finne kilde om mekanisk støy og legge inn motsetning mot synkronbelte}
...
\textbf{TODO: forklar extension slip (også kalt creep).}
...
\begin{table}[h]
	...
	\begin{center}
		\begin{tabular}[c]{l|l}
			...
			$J_m, J_L$                              & moment of inertia of motor and load \textbf{TODO: finn ut om effective eller ikke} \\
...
\textbf{TODO: finn ut om hvordan estimere EA}
...
\end{equation}
where $\dot{\theta}_{L,0}$ is the angular velocity of the operating point, and $\Psi$ is the relative damping of the belt which can be experimentally determined \textbf{TODO: finn ut hvordan}.
...
1932 by the French engineer Henri de Bellescize \cite{deBellescize1932ReceptionSynchrone}, but found its broad industrial application only after becoming available
...
\textbf{TODO: synchrous communication}

\textbf{TODO: \citep{Moore1973PLL} for circuit PLL for motors}

\section{Cascade Control}
\textbf{TODO: fortell om prinsipper for valg av båndbredde mellom loopsene osv}
...
\textbf{TODO: vis utsnitt av motoren}
\textbf{TODO: forklar de fysiske lovene som genererer dreiemoment.}

\subsection{Field Oriented Control}
\textbf{TODO: Introduser med problememene med pwm og sinus control}
...
MIDI messages consist of a status byte and then optionally up to two data bytes. The status byte identifies the message type and which MIDI channel the message is for. The optional data bytes contain the parameters for that message \cite{MMA1996}.
The status bytes and data bytes are separated by their most significant bit, status bytes using 1 and data bytes using 0.
Channel Voice messages are used to send information about a musical performance on a specific channel and can command stuff like:
...
System Real Time messages. The System Real Time messages are used to synchronize all clock-based equipment. They consist of only one status byte, and can be one of the following messages:
...
The timing clock messages are used to synchronize, sending the byte F8H 24 times per quarter note.
...
The Controller Area Network (CAN) protocol is a serial communication
protocol specified in ISO 11898-1~\cite{iso11898-1}. It employs a
non-destructive arbitration mechanism for media access control, which
limits the maximum bit rate to 1~Mbit/s and restricts the payload of a
single frame to 8~bytes~\cite{hartwich2012can}. The protocol includes
fault confinement mechanisms based on error frames, error counters, and
error-active/passive node states~\cite{hartwich2012can}.

In applications requiring higher throughput or larger data payloads per
frame, these limits can become a constraint, motivating the development
of an extended protocol.

\subsection{CAN with Flexible Data-Rate (CAN-FD)}

CAN with Flexible Data-Rate (CAN-FD) is an extension of the CAN protocol
developed by Bosch and presented in 2012, later incorporated into
ISO 11898-1~\cite{hartwich2012can,iso11898-1}. CAN-FD improves on
classical CAN in two ways: it supports bit rates higher than 1~Mbit/s,
and it supports payloads of more than 8~bytes per frame, with a new data
length coding allowing up to 64~bytes~\cite{hartwich2012can}.

CAN-FD retains the original CAN arbitration mechanism. After arbitration
is complete, the protocol switches to a shorter bit time for the data
phase of the frame, and switches back to the standard bit time before
the receivers transmit their acknowledge bit~\cite{hartwich2012can}.
All of CAN's fault confinement mechanisms --- including error frames,
error counters, and error-active/passive modes --- are
preserved~\cite{hartwich2012can}. CAN-FD protocol controllers are also
able to participate in standard CAN communication, allowing a gradual
introduction of CAN-FD nodes into existing CAN
systems~\cite{hartwich2012can}.

In this project, the moteus C1 motor controller communicates over CAN-FD.

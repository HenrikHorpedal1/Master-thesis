...
\begin{figure}[h]
	\centering
	\input{fig/background/leslie-inner-workings.tikz}
	\caption{Basic concept of a Leslie speaker. Redrawn from \citet{Leslie122Manual1971}.}
	\label{fig:leslie-inner-workings}
\end{figure}

The cabinet usually has three compartments. The upper one houses the treble rotor, the middle houses the speakers and crossover while the lower compartment houses the bass rotor and the amplifier.

Both speakers them self are stationary, but plays into rotating deflectors (check if both are called reflectors). The compression driver fires up into what starts as vertical tube and flares out making a horn. Although there appear to be two horns, only one is functional, the other is plugged and serves to balance the horn while it spins. A diffuser is located at the end of the horn which will be furthered discussed in section x.

The woofer fires downward into a rotating drum. The drum has straight walls and a scoop that starts vertically and ends horizontally.

Both rotors are driven with belts and pulleys by two AC induction motors, one for fast and one for slow.
(finn kilde på hastigheter og introduser chorale and tremolo)
...
\cite{kronland2008realtime}


\cite{TheatreOrgansMysteryLeslie} Explains the effect of a Leslie speakers as three main effects happening at once. Because the sound radiated out from the rotating deflector is directional, the intensity of the sound source from a listener's point will be varying as the deflector rotates. This effect is called Amplitude Modulation (AM).

\textbf{TODO: sett inn equation.}

However \cite{TheatreOrgansMysteryLeslie} as the sound source is rotating, its relative velocity to the listener will change, causing Frequency Modulation FM as a result of the Doppler Effect. As the deflector rotates towards the listener the frequency content of the source will be raised and as it rotates away it will be lowered.
...
The most common horn profile is the exponential horn \textbf{TODO: verifiser med kilde}, in which the cross-sectional area $S$ increases exponentially along the axis:
\begin{equation}
	S(x) = S_0\,e^{mx}
	\label{eq:horn-area}
\end{equation}
